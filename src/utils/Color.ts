...
export default class Color {
	 ...
	 * @returns {any} ближайший цвет или null
	 */
	static findSimilarColor(r: number, g: number, b: number, colors: number[], defaultColor: number): number {
		let sMin = Infinity
		let best = defaultColor
	...
	static calculateTransparentColor(fgHexColor: string, bgHexColor: string, opacity: number): string {
		// opacity = opacity * 1.0

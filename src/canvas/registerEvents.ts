import EventsHandler, {MovingWidgets} from 'canvas/EventsHandler'
...
export default function registerEvents(eventsHandler: EventsHandler, controller: CanvasController) {
	let shiftPressed = false
	let metaPressed = false
	let targetWidget: BaseWidget | undefined
	let movingWidgets: MovingWidgets = []
	let doSelection: boolean
	...
	controller.canvasContainer.addEventListener('mousedown', (e) => {
		...
		controlType = getControlType(e.target as any)
...
function getControlType(element: HTMLElement): ControlType | null {
	return element.getAttribute('control-type') as ControlType
}

define(["require", "exports"], function (require, exports) {
    ...
    class Color {
         ...
         */
        static findSimilarColorLab(r, g, b, colors, defaultColor, maxDiff = Infinity) {
        ...
        setColor(stringColor) {
            ...
            for (let key in stringParsers) {
                if (stringParsers.hasOwnProperty(key)) {
                    ...
                    let match = parser.re.exec(stringColor), values = match && parser.parse(match);
                    if (values) {
         ...
         * @returns {any} ближайший цвет или null
         */
        static findSimilarColor(r, g, b, colors, defaultColor) {
            let sMin = Infinity;
            let best = defaultColor;
            let kRed = 30;
            let kGreen = 59;
            ...
            colors.forEach(current => {
                ...
                let sCurrent = kRed * Math.pow(rgbCurrent.r - r, 2) +
        ...
        static findSimilarRGBColorByHSBCompare(red, green, blue, colors, defaultColor, hc, sc, bc) {
            const { h, s, b } = Color.RGBtoHSB(red, green, blue, 1);
            let sMin = Infinity;
            let best = defaultColor;
            colors.forEach(current => {
                let rgbCurrent = Color.intColorToRGB(current);
                let hsbCurrent = Color.RGBtoHSB(rgbCurrent.r, rgbCurrent.g, rgbCurrent.b, rgbCurrent.a);
                let sCurrent = hc * Math.pow(hsbCurrent.h - h, 2) +
        ...
        static intColorToHEX(intColor) {
            if (intColor === undefined) {
                return undefined;
            }
            else {
                return ('#' +
                    ((1 << 24) | (((intColor >> 16) & 255) << 16) | (((intColor >> 8) & 255) << 8) | (intColor & 255))
                        .toString(16)
                        .substr(1));
            }
        }
         ...
         */
        static HEXToNumberRGBA(hexString) {
            hexString = hexString.replace('#', '');
            if (hexString.length === 3) {
                // NOTE: duplicating every character twice, converting #fff to #ffffff to parse properly
                hexString = [hexString[0], hexString[0], hexString[1], hexString[1], hexString[2], hexString[2]].join('');
            }
        ...
        static calculateTransparentColor(fgHexColor, bgHexColor, opacity) {
            // opacity = opacity * 1.0
            ...
            return Color.intColorToHEX(Color.RGBtoNumber(finalRed, finalGreen, finalBlue)) || '';
        }
    }
